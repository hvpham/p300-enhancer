function runMultipleTest(result_file)
    ...
    nos_selected_kmers = [10,30];
    %nos_selected_kmers = [100,50,10];
    %nos_selected_kmers = [100];
    %nos_selected_kmers = [200];
    %nos_selected_kmers = [50];
    ...
    %svm_kmer_select_methods = {'2+-o','2-o'};
    %svm_kmer_select_methods = {'2+d'};
    n_folds = 5;
        ...
        for no=1:length(nos_selected_kmers)
            ...
            for cd=1:length(combination_distances)
                ...
                if ~check_if_done(done_tests,'enhancer_combined_svm', pos_file, neg_file,no_selected_kmers,kmers_length,combination_distance, preselect,'none',n_folds)
                    disp(['Running: ','enhancer_combined_svm(',pos_file,',',neg_file,',',num2str(no_selected_kmers),',',num2str(kmers_length),',',num2str(combination_distance),',',preselect,',o,',num2str(n_folds),')']);
                    a = enhancer_combined_svm(pos_file,neg_file,no_selected_kmers,kmers_length,combination_distance,preselect,'o',n_folds);
                    ...
                    store_data(result_file,'enhancer_combined_svm', pos_file, neg_file,no_selected_kmers,kmers_length,combination_distance, preselect,'o',n_folds, ROC_AUC,PR_AUC,weight);
                end
                %}

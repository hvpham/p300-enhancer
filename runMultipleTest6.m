function runMultipleTest5(result_file)
    ...
    %pos_files={'test1'};
    %pos_files={'1_TAL1_erythroid_pos_thanh','2_HNF4A_prolCell_pos_thanh','3_GATA6_prolCell_pos_thanh','4_CDX2_prolCell_pos_thanh','7_PHA4_embryo_pos_thanh','10_H3K4me2_prolCell_pos_thanh'};
    pos_files={'enh_fb','enh_lb','enh_mb'};
    ...
    %neg_files={'test2'};
    %neg_files={'1_TAL1_erythroid_neg_thanh','2_HNF4A_prolCell_neg_thanh','3_GATA6_prolCell_neg_thanh','4_CDX2_prolCell_neg_thanh','7_PHA4_embryo_neg_thanh','10_H3K4me2_prolCell_neg_thanh'};
    neg_files={'random4000','random4000','random4000'};
    ...
    %preselects1 = {'mRMR','adaboost'};
    %preselects2 = {'tTest','infoGain','Fisher','svm'};
    preselects2 = {'svm'};
    %preselects2 = {'tTest'};
    ...
    for fi=1:length(pos_files)
        ...
        for no=1:length(nos_selected_kmers)
            ...
            for cd=1:length(combination_distances)
                ...
                %{
                for prs=1:length(preselects1)
                    %preselect other
                    preselect = preselects1{prs};

                    if ~check_if_done(done_tests,'enhancer_combined_svm', pos_file, neg_file,no_selected_kmers,kmers_length,combination_distance, preselect,'2n',n_folds)
                        disp(['Running: ','enhancer_combined_svm(',pos_file,',',neg_file,',',num2str(no_selected_kmers),',',num2str(kmers_length),',',num2str(combination_distance),',',preselect,',2n,',num2str(n_folds),')']);
                        a = enhancer_combined_svm(pos_file,neg_file,no_selected_kmers,kmers_length,combination_distance,preselect,'2n',n_folds);
                        ...
                        store_data(result_file,'enhancer_combined_svm', pos_file, neg_file,no_selected_kmers,kmers_length,combination_distance, preselect,'2n',n_folds, ROC_AUC,PR_AUC,weight);
                    end
                end
                %}

                for prs=1:length(preselects2)
                    %preselect other
                    preselect = preselects2{prs};
